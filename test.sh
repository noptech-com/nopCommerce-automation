...
scp /tmp/nop_provision.sh "${SSH_TARGET}:${REMOTE_SCRIPT}" >/dev/null
ssh -t "${SSH_TARGET}" "sudo bash ${REMOTE_SCRIPT} '$DOMAIN' '$DB_NAME' '$DB_USER' '$DB_PASS' '$ADMIN_EMAIL' '$ADMIN_PASS_HASH' '$ADMIN_SALT'"
